...
public class Day06 : IDayPartOne, IDayPartTwo
{
    ...
    {
        var raceDurations = input[0]
            .Split(' ')
            .Where(v => int.TryParse(v, out _))
            .Select(int.Parse)
            .ToArray();
        var recordDistances = input[1]
            .Split(' ')
            .Where(v => int.TryParse(v, out _))
            .Select(int.Parse)
            .ToArray();
        ...
        return races.Select(r => r.NumberOfWaysToWin()).Aggregate(1, (a, b) => a * b);
    }

    public object PartTwo(string[] input)
    {
        var raceDuration = string.Join(
            "",
            input[0].Split(' ').Where(v => int.TryParse(v, out _)).Select(int.Parse).ToArray()
        );
        var recordDistance = string.Join(
            "",
            input[1].Split(' ').Where(v => int.TryParse(v, out _)).Select(int.Parse).ToArray()
        );

        return new BoatRace(
            long.Parse(raceDuration),
    ...
    private sealed class BoatRace
    {
        ...
        public int NumberOfWaysToWin()
        {
            ...
            for (var i = 0; i <= _duration; i++)
            {
                ...
                if (distanceTravelled > _recordDistance)
                    numberOfWaysToWin.Add(distanceTravelled);
            }
