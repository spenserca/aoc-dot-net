...
{
    private readonly Dictionary<string, string> _digitWordToStringValueMap = new()
    {
        ["one"] = "1",
        ["two"] = "2",
        ["three"] = "3",
    ...
    private const string DigitAndStringValuePattern = "(\\d)|(one|two|three|four|five|six|seven|eight|nine)";
    private readonly Regex _partTwoForwardRegex = new(DigitAndStringValuePattern);
    private readonly Regex _partTwoBackwardRegex = new(DigitAndStringValuePattern, RegexOptions.RightToLeft);
    ...
    public object PartOne(string[] input)
    {
        return input.Select(i =>
        {
            ...
            return int.Parse(GetNumericDigitAsString(forwardMatch.Value) +
